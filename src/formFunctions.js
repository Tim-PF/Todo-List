...
function importantButtonClick(
  importantButton,
  importantButtonTrue,
  currentProjectId,
) {
  const index = currentProjectId;
  ...
  const closestListItem = importantButton.closest("li");
...
function checkButtonClick(checkBoxDiv, currentProjectId) {
  ...
  const project = projectList[index].todos;
  // Finds Id of closest List item and with that the index to change important to true or false
  const closestListItem = checkBoxDiv.closest("li");
  const idValue = closestListItem.getAttribute("id");
...
function deleteButtonClicked(deleteButton, currentProjectId) {
  ...
  // Finds Id of closest List item and with that the index to change important to true or false
  const closestListItem = deleteButton.closest("li");
  const idValue = closestListItem.getAttribute("id");
  ...
  // Task to get removed
  const taskToRemove = project.find((item) => item.id === realIdValue);
